...
| **Phase 2** | 风险指标 | 5 | ✅ 全部完成 | 波动率/VaR/夏普/回撤 + 风险等级 |
...
**效果**：用户看到基金评分时，同时看到风险等级，避免只看收益不看风险。高波动率基金会标注为"高风险"。
...
- ✅ Phase 2: 风险指标模块（risk.py + API + 信号引擎集成 + FundDetail 风险画像卡片）
